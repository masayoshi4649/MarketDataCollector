...
kabus-controllerの登録一覧、板、ランキング、規制、銘柄・為替情報等へ直接接続する、認証不要のGoネイティブprovider設定である。Python設定は使用しない。銘柄指定の標準情報GETはAPI登録銘柄リストを変更し得る。
...
`base_url` は現在のLAN内エンドポイントを既定値とする。固定許可したGET以外へは接続せず、認証情報や取引操作を扱わない。ホスト、ネットワーク構成、ポートが変わった場合は、`default.toml` を直接編集せず、`conf/zz-kabus-controller.local.toml` など後順位のローカル設定でオリジンだけを上書きする。
